...
public class ApplyRegistration
{
	...
	static private class Params
	{
        ...
        @Parameter( names = "--registeredtilespec", description = "Json file containing stitching estimates", required = false)
	...
	public static void main( final String[] args )
	{
		...
		try
      	...
      	for (int i =0; i < inputtileSpecs.size(); i++)
      	{
      			...
      			/*ReferenceTransformSpec lts = (ReferenceTransformSpec)stitchedtileSpecs.get(stitchedindex).getLastTransform();
      			ReferenceTransformSpec newlts = new ReferenceTransformSpec(lts.getRefId());
      			ListTransformSpec mylist = inputtileSpecs.get(i).getTransforms();
      			mylist.addSpec(newlts);
      			inputtileSpecs.get(i).setTransforms(mylist);*/
      			
      			ListTransformSpec mylist = stitchedtileSpecs.get(stitchedindex).getTransforms();
      			inputtileSpecs.get(i).setTransforms(mylist);
